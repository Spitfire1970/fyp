...
class BatchProcessor:
    def __init__(self, save_path: str, batch_size: int = 1000):
        self.save_path = save_path
...
def main():
    ...
    with ProcessPoolExecutor() as executor:
        ...
        for pgn_file in tqdm(pgn_dir.glob("*.pgn.zst"), desc="Processing PGN files"):
            ...
            num_chunks = max(1, file_size // (50 * 1024 * 1024))  # 50MB chunks
            chunks = [(str(pgn_file), i * 1000, 1000) for i in range(num_chunks)]
            ...
            stats[str(pgn_file)] = stats.get(str(pgn_file), 0) + batch_size
